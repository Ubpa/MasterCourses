...
则 $\exist z \in V(G)\backslash\{x,y\}$，使得 $\{xy,z\}$ 是 $G\cdot xy$ 的分离集，则 $\{x,y,z\}$ 是 $G$ 的分离集

$x,y,z$ 都与 $G-\{x,y,z\}$ 的每个分支相连，取 $v\in N_G(z)\backslash\{x,y\}$，由假设有 $\kappa(G\cdot zv)=2$ 

则 $\exist w\in V(G)\backslash\{z,v\}$，使得 $\{zv,w\}$ 是 $G\cdot zv$ 的分离集，则 $\{z,v,w\}$ 是 $G$ 的分离集

然而 $x,y$ 连接每一个分支，则 $\{z,v,w\}$ 无法分离 $G\cdot zv$，矛盾
...
构造点 $y$ 与 $x_1,\dots,x_k$ 双向相邻，得到图 $D^\prime$，若 $\kappa(D^\prime)\le k-1$，则存在至多 $k-1$ 个点构成的点集 $\mathcal{S}$ 可分离 $D^\prime$，若 $y\in \mathcal{S}$，则 $\mathcal{S}\backslash\{y\}$ 分离 $D$，矛盾；若 $y\not\in \mathcal{S}$，则 $\mathcal{S}$ 分离 $D$，也矛盾。故 $\kappa(D^\prime)\ge k$。
