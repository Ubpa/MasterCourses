...
> $$
> [t_1]\frac{p(\cdot)}{x-\cdot}=\frac{a_0}{x-t_1}
> $$

- 假设 $n-1$ 时成立

> 设 $p(x)=\sum_{i=0}^{n-1}a_ix^{i}$ 
>
> - 当 $t_1=t_n$ 时
>
> > 当 $m=0,\dots,n-1$ 时
...
> > &=\frac{1}{(n-1)!}D^{n-1}_y\left(\frac{(x-(x-y))^m}{x-y}\right)\\
> > &=\frac{1}{(n-1)!}D^{n-1}_y\left(\frac{\sum_{i=0}^m C_n^i (-1)^{m-i} x^i (x-y)^{m-i}}{x-y}\right)\\
> > &=\frac{1}{(n-1)!}D^{n-1}_y\left(\frac{x^m}{x-y}+\sum_{i=1}^m C_n^i x^i (x-y)^{m-i-1}\right)\\
> > &=\frac{1}{(n-1)!}D^{n-1}_y\left(\frac{x^m}{x-y}\right)\\
> > &=\frac{x^m}{(x-y)^{n}}\\
> > \end{aligned}
> > $$
> > 则
...
> > &=\frac{1}{(n-1)!}D^{n-1}_y\left(\frac{p(y)}{x-y}\right)\Big|_{y=t_1}\\
> > &=\frac{1}{(n-1)!}D^{n-1}_y\left(\frac{\sum_{i=0}^{n-1} a_iy^i }{x-y}\right)\Big|_{y=t_1}\\
> > &=\sum_{i=0}^{n-1} a_i \frac{1}{(n-1)!} D^{n-1}_y\left(\frac{y^i }{x-y}\right)\Big|_{y=t_1}\\
> > &=\sum_{i=0}^{n-1} \frac{a_ix^i}{(x-t_1)^n}\\
> > &=\frac{p(x)}{(x-t_1)^n}\\
> > \end{aligned}
> > $$
>
...
[1,1,2,3]f &= \frac{[1,2,3]f-[1,1,2]f}{3-1}=-\frac{4}{105}\\
\end{aligned}
$$
