...
\item 红色的是 1) 的控制多边形
\item 蓝色圆点是 2) 的三个采样点
\item 绿色的多边形是 $u=3$ 处细分曲线的左部分的控制多边形，黄色和洋红色的多边形是右部分在中点细分的两曲线的控制多边形
\item 青色的是曲线 $P(u)$ 
\end{itemize}
...
\end{figure}
\begin{itemize}
\item 红色的圆点是 $P_0^{(0)},P_1^{(0)},P_2^{(0)},P_3  ^{(0)}$，红色虚线是相应控制多边形
\item 绿色的圆点是 $P_0^{(1)},P_1^{(1)},P_2^{(1)}$ 
\item 黄色的圆点是 $P_0^{(2)},P_1^{(2)}$ 
\item 青色的圆点是 $P_0^{(3)}$ 
\item 洋红色的圆点是 Bezier 曲线的控制点 $\pmb{b}_0,\pmb{b}_1,\pmb{b}_2,\pmb{b}_3$ 
\item 青色的实曲线是 Bezier 曲线
\end{itemize}
\subsubsection{1)}
$$
\begin{aligned}
