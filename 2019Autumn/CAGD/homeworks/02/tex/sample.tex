...
\pmb{p}_i &= a_i \pmb{u} + b_i \pmb{v} + c_i \pmb{w}, i = 0,1,\dots,n
\end{aligned}
\end{equation*}
因为控制顶点不共面，所以$c_i$不全为0。

由 $\pmb{p}(t)=\sum_{i=0}^n B^n_i(t)\pmb{p}_i$ 可得
$$
a\pmb{u} + b \pmb{v} = \left(\sum_{i=0}^n B^n_i(t) a_i\right)\pmb{u} + \left(\sum_{i=0}^n B^n_i(t) b_i\right)\pmb{v} + \left(\sum_{i=0}^n B^n_i(t) c_i\right)\pmb{w}
$$
则 $\sum_{i=0}^n B^n_i(t) c_i = 0$。由于 $\left\{B^n_i(t)\right\}_{i=0}^n$ 是一组基函数，所以 $c_i=0,i=0,1,\dots,n$，矛盾，则假设不成立，即控制顶点共面。

综上所述，一条空间\Bezier曲线为平面曲线的充要条件是其控制顶点共面。
\newpage
...
&= n\sum_{i=0}^n\sum_{j=0}^{n-1}(\pmb{p}_i\times(\pmb{p}_{j+1}-\pmb{p}_j))
   \frac{C_n^i C_{n-1}^j}{2nC_{2n-1}^{i+j}} \\
&= \frac{1}{2C_{2n-1}^n}\sum_{i=0}^n\sum_{j=0}^{n-1}
