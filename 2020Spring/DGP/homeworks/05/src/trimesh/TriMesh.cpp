...
void TriMesh::GenTexcoords() {
	texcoords.resize(positions.size());
	pointf3 center = pointf3::combine(positions, 1.f / positions.size());
	for (size_t i = 0; i < positions.size(); i++)
		texcoords.at(i) = (positions[i] - center).normalize().cast_to<normalf>().to_sphere_texcoord();
}
...
void TriMesh::CombineSamePositionVertex() {
	map<pointf3, uint32_t> pos2idx;
	for (const auto& pos : positions) {
		if(pos2idx.find(pos) != pos2idx.end())
			continue;

		pos2idx.emplace(pos, static_cast<uint32_t>(pos2idx.size()));
	}

	for (auto& triangle : indices) {
		triangle[0] = pos2idx[positions[triangle[0]]];
		triangle[1] = pos2idx[positions[triangle[1]]];
		triangle[2] = pos2idx[positions[triangle[2]]];
	}

	positions.resize(pos2idx.size());

	for (const auto& [pos, idx] : pos2idx)
		positions[idx] = pos;
